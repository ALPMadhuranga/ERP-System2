...
import { adminAddDekan } from "../../redux/action/adminAction";
import AdminDekanDetails from "./AdminDekanDetail";
import { useTranslation } from "react-i18next";

function AdminGetAllDekan() {
	...
	const list = fetched.map((row) => {
		...
		const firstNumber = row.firstNumber;
		const secontNumber = row.secontNumber;
	...
	const deleteHandler = async (row) => {
		...
		if (
			window.confirm(
				`Are you sure ${row.getValue("registrationNumber")} delete?`
			)
		) {
			try {
				await axios({
					method: "Post",
					url: "/api/admin/deleteDekan",
					data: regnum,
				});
				toast.success(`Subject ${row.getValue("registrationNumber")} deleted`);
				getAllMovies();
			} catch (err) {
				console.log(err);
			}
	...
	const resetHandler = async (registrationNumber) => {
		...
		if (
			window.confirm(`Are you sure ${registrationNumber} reset the password?`)
		) {
			try {
				await axios({
					method: "Post",
					url: "/api/admin/resetPasswordDekan",
					data: id,
				});
									...
									<TableContainer
										...
										<MaterialReactTable
											...
											renderBottomToolbarCustomActions={({ table }) => (
												<>
													{t("total_page")} {table.getRowModel().rows.length}
													<Typography>
														{t("total_deans")}
														{list.length}
													</Typography>
...
export const CreateNewStudent = ({ open, onClose }) => {
	...
	useEffect(() => {
		if (store.error || store.admin.adminAddDekanFlag) {
			setIsLoading(false);
		}
	}, [store.error, store.admin.adminAddDekanFlag]);
	return (
		<Dialog open={open}>
			<DialogTitle textAlign="center">{t("add_dekan")}</DialogTitle>
			<DialogContent>
...
export const CreateNewAccountModal = ({ open, onClose, getAllMovies }) => {
	...
	return (
		<Dialog open={open}>
			<DialogTitle textAlign="center">{t("update_deans")}</DialogTitle>
			<DialogContent>
				<AdminDekanDetails getAllMovies={getAllMovies} onClose={onClose} />
			</DialogContent>
		</Dialog>
	);
};

export default AdminGetAllDekan;
