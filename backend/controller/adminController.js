...
module.exports = {
  ...
  updatePassword: async (req, res, next) => {
    try {
      ...
      if (!isCorrect) {
        errors.oldPassword = "The error is an old password";
        return res.status(404).json(errors);
      }
      let hashedPassword;
    ...
    try {
      ...
      if (!name || !contactNumber || !dob) {
        return res.status(400).json({
          success: false,
          message: "Maybe you missed some fields!",
        });
      }
  ...
  getNewsAdmin: async (req, res) => {
    try {
      ...
      if (dekans.length === 0) {
        error = "News Admin Not available!";
  ...
  updateNewsAdmin: async (req, res, next) => {
    ...
    try {
      if (!isValid) {
        ...
        res.status(200).json({ message: "Dean updated successfully", data });
      }
    } catch (error) {
  ...
  addDekan: async (req, res, next) => {
    try {
      ...
      return res.status(200).json({
        success: true,
        message: "Dean registerd successfully",
        result: newDekan,
      });
    } catch (err) {
      return res.status(400).json({
  ...
  addSubject: async (req, res, next) => {
    try {
      ...
      if (subject) {
        errors.subjectCode = "Science added";
        return res.status(400).json(errors);
      }
      ...
      if (students.length === 0) {
        errors.department = "Science added";
        return res.status(400).json(errors);
      } else {
  ...
  getAllSubject: async (req, res, next) => {
    try {
      ...
      if (!Department) {
        errors.department = "Faculty not found!";
        return res.status(404).json(errors);
      }
  ...
  checkAttendence: async (req, res, next) => {
    try {
      ...
      console.log("Error in fetching check Attendence", err.message);
    }
  },
};