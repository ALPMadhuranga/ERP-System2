...
module.exports = {
	fetchStudents: async (req, res, next) => {
		try {
			...
			console.log("error in faculty fetch students", err.message);
		}
	},
	uploadMarks: async (req, res, next) => {
		try {
			...
			if (isAlready.length !== 0) {
				errors.exam = "You have already uploaded the exam grades!";
				return res.status(400).json(errors);
			}
			...
			res.status(200).json({ message: "Evaluation has been loaded!" });
		} catch (err) {
	...
	updateSubjects: async (req, res, next) => {
		try {
			...
			if (subject) {
				errors.subjectName = "Science Available";
				return res.status(400).json(errors);
			}
	...
	addStudent: async (req, res, next) => {
		try {
			...
			if (regnum) {
				errors.registrationNumber = "Reg Number available";
				return res.status(400).json(errors);
			}
	...
	addSubject: async (req, res, next) => {
		try {
			...
			if (students.length === 0) {
				errors.department = "There are currently no students available!";
				return res.status(400).json(errors);
			} else {
				...
				res.status(200).json({ newSubject, message: "Science Added" });
			}
		} catch (err) {
